...
-- Description: Self‑checking testbench for the RISC‑V control unit.
--              Tests all instruction types (opcodes, funct3, funct7) and
--              branch conditions by comparing DUT outputs against a reference
--              model. Includes corner cases and random vectors.
-------------------------------------------------------------------------------
...
architecture sim of tb_control_unit is
  ...
  -----------------------------------------------------------------------------
  -- Reference model: returns a record of all control outputs
  -----------------------------------------------------------------------------
  ...
  begin
    ...
    result.aluop := ALU_OP_TYPE_ADD; -- default
    if (opcode = INSTR_OP_REG_REG and funct3 = INSTR_F3_SUB and funct7 = INSTR_F7_SUB) or
       (opcode = INSTR_OP_BRANCH and (funct3 = INSTR_F3_BEQ or funct3 = INSTR_F3_BNE)) then
      result.aluop := ALU_OP_TYPE_SUB;
    elsif (opcode = INSTR_OP_REG_IMM and funct3 = INSTR_F3_SLTI) or
          (opcode = INSTR_OP_REG_REG and funct3 = INSTR_F3_SLT and funct7 = INSTR_F7_SLT) or
          (opcode = INSTR_OP_BRANCH and (funct3 = INSTR_F3_BLT or funct3 = INSTR_F3_BGE)) then
      result.aluop := ALU_OP_TYPE_SLT;
    elsif (opcode = INSTR_OP_REG_IMM and funct3 = INSTR_F3_SLTIU) or
          (opcode = INSTR_OP_REG_REG and funct3 = INSTR_F3_SLTU and funct7 = INSTR_F7_SLTU) or
          (opcode = INSTR_OP_BRANCH and (funct3 = INSTR_F3_BLTU or funct3 = INSTR_F3_BGEU)) then
      result.aluop := ALU_OP_TYPE_SLTU;
    elsif (opcode = INSTR_OP_REG_IMM and funct3 = INSTR_F3_ANDI) or
          (opcode = INSTR_OP_REG_REG and funct3 = INSTR_F3_AND and funct7 = INSTR_F7_AND) then
      result.aluop := ALU_OP_TYPE_AND;
    elsif (opcode = INSTR_OP_REG_IMM and funct3 = INSTR_F3_ORI) or
          (opcode = INSTR_OP_REG_REG and funct3 = INSTR_F3_OR and funct7 = INSTR_F7_OR) then
      result.aluop := ALU_OP_TYPE_OR;
    elsif (opcode = INSTR_OP_REG_IMM and funct3 = INSTR_F3_XORI) or
          (opcode = INSTR_OP_REG_REG and funct3 = INSTR_F3_XOR and funct7 = INSTR_F7_XOR) then
      result.aluop := ALU_OP_TYPE_XOR;
    elsif (opcode = INSTR_OP_REG_IMM and funct3 = INSTR_F3_SLLI and funct7 = INSTR_F7_SLLI) or
          (opcode = INSTR_OP_REG_REG and funct3 = INSTR_F3_SLL and funct7 = INSTR_F7_SLL) then
      result.aluop := ALU_OP_TYPE_SLL;
    elsif (opcode = INSTR_OP_REG_IMM and funct3 = INSTR_F3_SRLI and funct7 = INSTR_F7_SRLI) or
          (opcode = INSTR_OP_REG_REG and funct3 = INSTR_F3_SRL and funct7 = INSTR_F7_SRL) then
      result.aluop := ALU_OP_TYPE_SRL;
    elsif (opcode = INSTR_OP_REG_IMM and funct3 = INSTR_F3_SRAI and funct7 = INSTR_F7_SRAI) or
          (opcode = INSTR_OP_REG_REG and funct3 = INSTR_F3_SRA and funct7 = INSTR_F7_SRA) then
      result.aluop := ALU_OP_TYPE_SRA;
    end if;

    -- ALU operand B source (use immediate)
    if opcode = INSTR_OP_REG_IMM or opcode = INSTR_OP_JALR or
       opcode = INSTR_OP_LOAD or opcode = INSTR_OP_STORE then
      result.alu_use_imm := '1';
    end if;

    -- Register write enable
    if opcode = INSTR_OP_BRANCH or opcode = INSTR_OP_STORE or
       opcode = INSTR_OP_FENCE or opcode = INSTR_OP_SYSTEM then
      result.write_rd := '0';
    end if;
    ...
    if opcode = INSTR_OP_BRANCH then
      if (branch = BRANCH_TYPE_BEQ and alu_result = x"00000000") or
         (branch = BRANCH_TYPE_BNE and alu_result /= x"00000000") or
         (branch = BRANCH_TYPE_BLT and alu_result = x"00000001") or
         (branch = BRANCH_TYPE_BGE and alu_result = x"00000000") then
    ...
    -- Memory access width
    if (opcode = INSTR_OP_LOAD  and funct3 = INSTR_F3_LW) or
       (opcode = INSTR_OP_STORE and funct3 = INSTR_F3_SW) then
      result.mem_access_width := MEM_ACCESS_WIDTH_32;
    elsif (opcode = INSTR_OP_LOAD  and (funct3 = INSTR_F3_LH or funct3 = INSTR_F3_LHU)) or
          (opcode = INSTR_OP_STORE and funct3 = INSTR_F3_SH) then
  ...
  process
    ...
    begin
      ...
      if alu_use_imm /= expected.alu_use_imm then
        ...
        write(l, alu_use_imm); write(l, string'(" exp=")); write(l, expected.alu_use_imm);
        report l.all severity error; deallocate(l); error_cnt := error_cnt + 1;
      end if;

      if write_rd /= expected.write_rd then
        write(l, string'("Mismatch: " & description & " write_rd="));
        write(l, write_rd); write(l, string'(" exp=")); write(l, expected.write_rd);
        report l.all severity error; deallocate(l); error_cnt := error_cnt + 1;
      end if;

      if write_mem /= expected.write_mem then
        write(l, string'("Mismatch: " & description & " write_mem="));
        write(l, write_mem); write(l, string'(" exp=")); write(l, expected.write_mem);
        report l.all severity error; deallocate(l); error_cnt := error_cnt + 1;
      end if;

      if take_branch /= expected.take_branch then
        write(l, string'("Mismatch: " & description & " take_branch="));
        write(l, take_branch); write(l, string'(" exp=")); write(l, expected.take_branch);
        report l.all severity error; deallocate(l); error_cnt := error_cnt + 1;
      end if;

      if rd_data_src /= expected.rd_data_src then
        write(l, string'("Mismatch: " & description & " rd_data_src="));
        write(l, RD_DATA_SRC_t'image(rd_data_src));   -- corrected
        write(l, string'(" exp="));
      ...
      if jump /= expected.jump then
        ...
        write(l, jump); write(l, string'(" exp=")); write(l, expected.jump);
        report l.all severity error; deallocate(l); error_cnt := error_cnt + 1;
      end if;

      if aluop /= expected.aluop then
        write(l, string'("Mismatch: " & description & " aluop="));
        write(l, ALU_OP_TYPE_t'image(aluop));         -- corrected
        write(l, string'(" exp="));
      ...
      if mem_access_width /= expected.mem_access_width then
        ...
        write(l, MEM_ACCESS_WIDTH_t'image(mem_access_width));   -- corrected
        write(l, string'(" exp="));
      ...
      if mem_access /= expected.mem_access then
        ...
        write(l, mem_access); write(l, string'(" exp=")); write(l, expected.mem_access);
        report l.all severity error; deallocate(l); error_cnt := error_cnt + 1;
      end if;
    end procedure;

    ---------------------------------------------------------------------------
    -- Helper to generate random 32‑bit value
    ---------------------------------------------------------------------------
