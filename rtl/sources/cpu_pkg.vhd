...
package cpu_pkg is
  ...
  -- Memory access width enumeration

  type MEM_ACCESS_WIDTH_t is (
      MEM_ACCESS_WIDTH_8,       -- Byte access                (8 bits)
      MEM_ACCESS_WIDTH_16,      -- Halfword access            (16 bits)
      MEM_ACCESS_WIDTH_32       -- Word access                (32 bits)
  );

  -- PC next source enumeration
 
  type PC_NEXT_SRC_t is (
      PC_NEXT_SRC_PC_ALU_RES,   -- Result from ALU            (e.g., for JALR)
      PC_NEXT_SRC_PC_IMM,       -- PC + immediate             (For JAL, branches)
      PC_NEXT_SRC_PC_4          -- PC + 4                     (Normal sequential fetch)
  );
  ...
  type BRANCH_TYPE_t is (
      ...
      BRANCH_TYPE_BEQ,          -- Branch if equal            (rs1 == rs2)
      BRANCH_TYPE_BNE,          -- Branch if not equal        (rs1 != rs2)
      BRANCH_TYPE_BLT,          -- Branch if less than        (Signed)
      BRANCH_TYPE_BGE           -- Branch if greater or equal (Signed)
  );

  -- Register destination data source enumeration

  type RD_DATA_SRC_t is (
      RD_DATA_SRC_PC_IMM,       -- PC + immediate             (AUIPC)
      RD_DATA_SRC_PC_4,         -- PC + 4                     (JAL, JALR link)
      RD_DATA_SRC_IMM,          -- Immediate value            (LUI)
      RD_DATA_SRC_ALU_RESULT,   -- Result from ALU            (Mostly arithmetic)
      RD_DATA_SRC_MEM_DATA_OUT  -- Data read from memory      (LOAD)
  );

  -- ALU operation type enumeration

  type ALU_OP_TYPE_t is (
      ALU_OP_TYPE_ADD,          -- Addition
      ALU_OP_TYPE_SUB,          -- Subtraction
      ALU_OP_TYPE_SLT,          -- Set less than              (Signed)
      ALU_OP_TYPE_SLTU,         -- Set less than              (Unsigned)
      ALU_OP_TYPE_AND,          -- Bitwise AND
      ALU_OP_TYPE_OR,           -- Bitwise OR
      ALU_OP_TYPE_XOR,          -- Bitwise XOR
      ALU_OP_TYPE_SLL,          -- Logical left shift
      ALU_OP_TYPE_SRL,          -- Logical right shift
      ALU_OP_TYPE_SRA,          -- Arithmetic right shift
      ALU_OP_TYPE_PASS          -- Pass operand A unchanged   (Used for LUI/AUIPC)
  );

  -- ALU source selection enumeration

  type ALU_OP_SRC_t is (
      ALU_OP_SRC_IMM,          -- Immediate value             (From decoder)
      ALU_OP_SRC_ALU_RES,      -- ALU result                  (For some addressing modes, e.g., JALR)
      ALU_OP_SRC_REG,          -- Register value              (rs1 or rs2)
      ALU_OP_SRC_PC_IMM,       -- PC + immediate              (AUIPC)
      ALU_OP_SRC_PC_4,         -- PC + 4                      (JAL/JALR)
      ALU_OP_SRC_RD_DATA       -- Data to be written back     (Used for forwarding)
  );

  -- NOP instruction constant

  constant INSTR_NOP           : std_logic_vector(31 downto 0) := x"00000013";

  -- Opcode constants (RV32I base instruction set)
  ...
  constant INSTR_OP_REG_IMM    : std_logic_vector(6 downto 0)  := "0010011";  -- Register‑immediate ALU ops
  constant INSTR_OP_REG_REG    : std_logic_vector(6 downto 0)  := "0110011";  -- Register‑register ALU ops
  constant INSTR_OP_FENCE      : std_logic_vector(6 downto 0)  := "0001111";  -- FENCE instruction
  ...
  constant INSTR_F3_SUB        : std_logic_vector(2 downto 0)  := "000";      -- Same as ADD, distinguished by funct7
  ...
  constant INSTR_F3_SRA        : std_logic_vector(2 downto 0)  := "101";      -- Same as SRL, distinguished by funct7
  constant INSTR_F3_OR         : std_logic_vector(2 downto 0)  := "110";
  constant INSTR_F3_AND        : std_logic_vector(2 downto 0)  := "111";

  -- For immediate ALU operations (register‑immediate)
  ...
  constant INSTR_F3_SRAI       : std_logic_vector(2 downto 0)  := "101";      -- Same as SRLI, distinguished by funct7
  ...
  constant INSTR_F3_BLTU       : std_logic_vector(2 downto 0)  := "110";      -- Unsigned version (not used directly in BRANCH_TYPE_t)
  constant INSTR_F3_BGEU       : std_logic_vector(2 downto 0)  := "111";

  -- For fence and system instructions (funct3 is usually 000)
  ...
  constant INSTR_F7_SRAI       : std_logic_vector(6 downto 0)  := "0100000";  -- bit 30 = 1 for arithmetic shift

  constant INSTR_F7_ADD        : std_logic_vector(6 downto 0)  := "0000000";
  constant INSTR_F7_SUB        : std_logic_vector(6 downto 0)  := "0100000";  -- bit 30 = 1 for subtract
  ...
  constant INSTR_F7_SRA        : std_logic_vector(6 downto 0)  := "0100000";  -- bit 30 = 1 for arithmetic shift
