...
void Vtb_memory_stage__Syms::_traceDump() {
    const VerilatedLockGuard lock(__Vm_dumperMutex);
    __Vm_dumperp->dump(VL_TIME_Q());
}

void Vtb_memory_stage__Syms::_traceDumpOpen() {
    const VerilatedLockGuard lock(__Vm_dumperMutex);
...
void Vtb_memory_stage__Syms::_traceDumpClose() {
    const VerilatedLockGuard lock(__Vm_dumperMutex);
    __Vm_dumping = false;
    ...
    , TOP__tb_memory_stage__DOT__wb{this, Verilated::catName(namep, "tb_memory_stage.wb")}
{
    ...
    // Setup scopes
    __Vscope_tb_memory_stage.configure(this, name(), "tb_memory_stage", "tb_memory_stage", -9, VerilatedScope::SCOPE_OTHER);
}
